...
fn installs_local_skill_and_updates_from_source() {
    ...
    let app = crate::library::LibraryPaths::new(app_dir.path().to_path_buf());
    let (_dir, store) = make_store();
    ...
    let res = super::install_local_skill(&app, &store, source.path(), Some("local1".to_string()))
        .unwrap();
    ...
    let up = super::update_managed_skill_from_source(&app, &store, &res.skill_id).unwrap();
    ...
    let err =
        match super::install_local_skill(&app, &store, source.path(), Some("local1".to_string())) {
...
fn failed_update_marks_skill_error_and_success_clears_it() {
    ...
    let error = match super::update_managed_skill_from_source(&app, &store, &skill.id) {
    ...
    super::update_managed_skill_from_source(&app, &store, &skill.id).unwrap();
    assert_eq!(
        store.get_skill_by_id(&skill.id).unwrap().unwrap().status,
        "ok"
    );
    ...
    store.upsert_skill_target(&target).unwrap();
    assert!(super::update_managed_skill_from_source(&app, &store, &skill.id).is_err());
    assert_eq!(
        store.get_skill_by_id(&skill.id).unwrap().unwrap().status,
        "ok"
    );
...
fn imports_identical_existing_local_skill_but_rejects_different_content() {
    ...
    let installed =
        super::install_local_skill(&app, &store, original.path(), Some("local1".to_string()))
            .unwrap();
...
fn lists_and_installs_git_skills_without_network() {
    ...
    let candidates =
        super::list_git_skills(&app, &store, repo_dir.path().to_string_lossy().as_ref()).unwrap();
...
fn install_local_selection_validates_skill_md() {
    ...
    let res =
        super::install_local_skill_from_selection(&app, &store, base.path(), "skills/a", None)
            .unwrap();
    assert!(res.central_path.exists());
    let skill = store.get_skill_by_id(&res.skill_id).unwrap().unwrap();
