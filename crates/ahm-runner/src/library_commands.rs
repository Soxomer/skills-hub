...
use crate::central_repo::{ensure_central_repo, resolve_central_repo_path};
use crate::content_hash::hash_dir;
use crate::featured_skills::{fetch_featured_skills, FeaturedSkill};
use crate::github_search::{search_github_repos, RepoSummary};
use crate::installer::{
...
use crate::onboarding::{
    get_discovery_scan_settings as get_discovery_scan_settings_core, save_discovery_scan_config,
    DiscoveryScanConfig, DiscoveryScanSettings, OnboardingPlan,
};
use crate::skill_store::{SkillRecord, SkillStore, SkillTargetRecord};
use crate::skills_search::{search_skills_online as search_skills_online_core, OnlineSkillResult};
...
use crate::tool_adapters::{
    adapter_by_key, adapters_sharing_project_skills_dir, is_builtin_tool_enabled, load_tool_config,
    project_relative_skills_dir, save_tool_config, supports_project_scope, CustomToolConfig,
    ToolConfig,
};
use uuid::Uuid;
...
fn runtime_tools(store: &SkillStore, include_disabled: bool) -> anyhow::Result<Vec<RuntimeTool>> {
    ...
    for adapter in crate::tool_adapters::default_tool_adapters() {
        let enabled = is_builtin_tool_enabled(&config, adapter.id.as_key());
        if !include_disabled && !enabled {
            continue;
        }
        let home = crate::library::LibraryPaths::from_store(store)?.home;
        let detected = home.join(adapter.relative_detect_dir).exists();
        tools.push(RuntimeTool {
            ...
            enabled,
            is_custom: false,
            skills_dir: home.join(adapter.relative_skills_dir),
...
pub fn get_tool_config(store: SkillStore) -> Result<ToolConfigDto, String> {
    let store = store.clone();
    load_tool_config(&store)
        .map(ToolConfigDto::from)
        .map_err(format_anyhow_error)
}

pub fn set_tool_config(store: SkillStore, config: ToolConfigDto) -> Result<ToolConfigDto, String> {
    let store = store.clone();
    save_tool_config(&store, config.into())
        .map(ToolConfigDto::from)
        .map_err(format_anyhow_error)
}

pub fn get_tool_status(store: SkillStore) -> Result<ToolStatusDto, String> {
    let store = store.clone();
    (move || {
...
pub fn get_onboarding_plan(
    app: crate::library::LibraryPaths,
    store: SkillStore,
) -> Result<OnboardingPlan, String> {
    let store = store.clone();
    (move || {
        crate::onboarding::build_onboarding_plan_for_home(
            &store,
            &app.home,
            &crate::central_repo::resolve_central_repo_path_for_home(&store, &app.home)?,
        )
    })()
    .map_err(format_anyhow_error)
}

pub fn get_discovery_scan_settings(store: SkillStore) -> Result<DiscoveryScanSettings, String> {
    let store = store.clone();
    get_discovery_scan_settings_core(&store).map_err(format_anyhow_error)
}

pub fn set_discovery_scan_config(
    store: SkillStore,
...
pub fn get_git_cache_cleanup_days(store: SkillStore) -> Result<i64, String> {
    let store = store.clone();
    Ok::<_, anyhow::Error>(get_git_cache_cleanup_days_core(&store)).map_err(format_anyhow_error)
}

pub fn set_git_cache_cleanup_days(store: SkillStore, days: i64) -> Result<i64, String> {
    let store = store.clone();
    set_git_cache_cleanup_days_core(&store, days).map_err(format_anyhow_error)
}

pub fn clear_git_cache_now(app: crate::library::LibraryPaths) -> Result<usize, String> {
    cleanup_git_cache_dirs(&app, std::time::Duration::from_secs(0)).map_err(format_anyhow_error)
}

pub fn get_git_cache_ttl_secs(store: SkillStore) -> Result<i64, String> {
    let store = store.clone();
    Ok::<_, anyhow::Error>(get_git_cache_ttl_secs_core(&store)).map_err(format_anyhow_error)
}

pub fn set_git_cache_ttl_secs(store: SkillStore, secs: i64) -> Result<i64, String> {
    let store = store.clone();
    set_git_cache_ttl_secs_core(&store, secs).map_err(format_anyhow_error)
}
...
pub fn get_auto_update_config(store: SkillStore) -> Result<AutoUpdateConfigDto, String> {
    let store = store.clone();
    get_auto_update_config_core(&store)
        .map(to_auto_update_config_dto)
        .map_err(format_anyhow_error)
}
...
pub fn run_auto_update_now(
    app: crate::library::LibraryPaths,
    store: SkillStore,
) -> Result<AutoUpdateRunResultDto, String> {
    let store = store.clone();
    run_auto_update_now_core(&app, &store)
        .map(to_auto_update_run_result_dto)
        .map_err(format_anyhow_error)
}

pub fn trigger_auto_update_task_now_cmd(store: SkillStore) -> Result<(), String> {
    let store = store.clone();
    (move || {
        record_auto_update_triggered(&store)?;
        let paths = crate::library::LibraryPaths::from_store(&store)?;
        run_auto_update_now_core(&paths, &store).map(|_| ())
    })()
    .map_err(format_anyhow_error)
}
...
pub fn get_recent_projects(store: SkillStore) -> Result<Vec<String>, String> {
    let store = store.clone();
    get_recent_projects_impl(&store).map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn save_recent_project(store: SkillStore, projectPath: String) -> Result<Vec<String>, String> {
    let store = store.clone();
    save_recent_project_impl(&store, &projectPath).map_err(format_anyhow_error)
}
...
pub fn set_central_repo_path(
    ...
    (move || {
        ...
        let current_base = current_base.canonicalize()?;
        let new_base = new_base.canonicalize()?;
        anyhow::ensure!(
            !new_base.starts_with(&current_base) && !current_base.starts_with(&new_base),
        ...
        for skill in &skills {
            crate::library::check_owned_targets(&store, &skill.id)?;
            let old = std::path::PathBuf::from(&skill.central_path);
            anyhow::ensure!(old.is_dir(), "Central skill folder is missing");
            let new = new_base.join(old.file_name().context("Invalid skill folder")?);
            anyhow::ensure!(!new.exists(), "The destination already contains this skill");
            for target in store.list_skill_targets(&skill.id)? {
                if target.status != "disabled"
                    && target.mode != "copy"
                    && std::path::Path::new(&target.target_path).exists()
            ...
            moves.push((skill.clone(), old, new));
        }
        // Keep every original until copies, links and records have all succeeded.
        ...
        let migrated = (|| -> anyhow::Result<()> {
            ...
            for (target, old, new) in &links {
                relinked.push((target.clone(), old.clone()));
                sync_dir_with_mode_with_overwrite(SyncMode::Auto, new, target, true)?;
            }
            for (skill, _, new) in &moves {
                let mut updated = skill.clone();
                updated.central_path = new.to_string_lossy().into_owned();
        ...
        if let Err(error) = migrated {
            for (target, old) in relinked.iter().rev() {
                sync_dir_with_mode_with_overwrite(SyncMode::Auto, old, target, true)?;
            }
            for (skill, _, _) in &moves {
                store.upsert_skill(skill)?;
            }
            for path in copied {
                crate::sync_engine::remove_path_any(&path)?;
            }
            return Err(error);
        }
        for (_, old, _) in moves {
            crate::sync_engine::remove_path_any(&old)?;
        }
...
#[allow(non_snake_case)]
pub fn list_local_skills_cmd(basePath: String) -> Result<Vec<LocalSkillCandidate>, String> {
    {
...
#[allow(non_snake_case)]
pub fn install_local_selection(
    app: crate::library::LibraryPaths,
    store: SkillStore,
    basePath: String,
    subpath: String,
...
pub fn install_git(
    ...
    let store = store.clone();
    cancel.reset();
    let cancel_token = Arc::clone(&cancel);
    (move || {
...
#[allow(non_snake_case)]
pub fn list_git_skills_cmd(
    app: crate::library::LibraryPaths,
    store: SkillStore,
    repoUrl: String,
) -> Result<Vec<GitSkillCandidate>, String> {
    let store = store.clone();
    list_git_skills(&app, &store, &repoUrl).map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn preview_git_skill_cmd(
    store: SkillStore,
    repoUrl: String,
    skillName: Option<String>,
) -> Result<GitSkillPreview, String> {
    let store = store.clone();
    preview_git_skill(&store, &repoUrl, skillName.as_deref()).map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn install_git_selection(
    app: crate::library::LibraryPaths,
    store: SkillStore,
    repoUrl: String,
    subpath: String,
...
pub fn sync_skill_dir(source_path: String, target_path: String) -> Result<SyncResultDto, String> {
    (move || {
        ...
        let group_tool_keys: Vec<String> =
            if let Ok(runtime_tool) = runtime_tool_by_key(&store, &tool) {
                ...
                let group = if scope == "project" {
                    ...
                    crate::tool_adapters::adapters_sharing_skills_dir(&adapter)
                };
                // If none of the group tools are installed, do nothing (treat as already not effective).
                if scope == "global" {
                    let mut any_installed = false;
                    for a in &group {
                        if crate::library::LibraryPaths::from_store(&store)?
                            .home
                            .join(a.relative_detect_dir)
                            .exists()
                        {
                            any_installed = true;
                            break;
...
#[allow(non_snake_case)]
pub fn set_skill_enabled(store: SkillStore, skillId: String, enabled: bool) -> Result<(), String> {
    let store = store.clone();
    (move || {
        if !enabled {
...
pub fn get_github_token(store: SkillStore) -> Result<String, String> {
    let store = store.clone();
    (move || Ok::<_, anyhow::Error>(store.get_setting("github_token")?.unwrap_or_default()))()
        .map_err(format_anyhow_error)
}

pub fn set_github_token(store: SkillStore, token: String) -> Result<(), String> {
    let store = store.clone();
    (move || {
...
pub fn get_github_proxy_config(store: SkillStore) -> Result<GithubProxyConfigDto, String> {
    let store = store.clone();
    get_github_proxy_config_core(&store)
        .map(to_github_proxy_config_dto)
        .map_err(format_anyhow_error)
}
...
pub fn set_github_proxy_config(
    ...
    let store = store.clone();
    set_github_proxy_config_core(&store, enabled, port)
        .map(to_github_proxy_config_dto)
        .map_err(format_anyhow_error)
}

pub fn get_github_proxy_url(store: SkillStore) -> Result<String, String> {
    let store = store.clone();
    get_github_proxy_url_core(&store).map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn set_github_proxy_url(store: SkillStore, proxyUrl: String) -> Result<String, String> {
    let store = store.clone();
    set_github_proxy_url_core(&store, &proxyUrl).map_err(format_anyhow_error)
}
...
pub fn get_managed_skills(store: SkillStore) -> Result<Vec<ManagedSkillDto>, String> {
    get_managed_skills_impl(&store)
}

pub fn get_tags(store: SkillStore) -> Result<Vec<TagWithCountDto>, String> {
    store
...
#[allow(non_snake_case)]
pub fn create_tag(store: SkillStore, name: String) -> Result<TagDto, String> {
    store
        .create_tag(&name)
        .map(|tag| TagDto {
            id: tag.id,
...
#[allow(non_snake_case)]
pub fn rename_tag(store: SkillStore, tagId: i64, name: String) -> Result<TagDto, String> {
    store
...
#[allow(non_snake_case)]
pub fn delete_tag(store: SkillStore, tagId: i64) -> Result<(), String> {
    store.delete_tag(tagId).map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn get_skill_tags(store: SkillStore, skillId: String) -> Result<Vec<TagDto>, String> {
    store
...
#[allow(non_snake_case)]
pub fn set_skill_tags(store: SkillStore, skillId: String, tagIds: Vec<i64>) -> Result<(), String> {
    store
        .set_skill_tags(&skillId, &tagIds)
        .map_err(format_anyhow_error)
}

pub fn get_untagged_skill_ids(store: SkillStore) -> Result<Vec<String>, String> {
    store.list_untagged_skill_ids().map_err(format_anyhow_error)
}

#[allow(non_snake_case)]
pub fn delete_managed_skill(store: SkillStore, skillId: String) -> Result<(), String> {
    let store = store.clone();
    (move || {
...
pub(crate) fn to_install_dto(result: InstallResult) -> InstallResultDto {
    InstallResultDto {
...
fn to_auto_update_config_dto(config: AutoUpdateConfig) -> AutoUpdateConfigDto {
    AutoUpdateConfigDto {
        ...
        task_registered: config.enabled,
        task_status_detail: "Scheduled by the connected runner".to_string(),
...
pub fn get_featured_skills(store: SkillStore) -> Result<Vec<FeaturedSkillDto>, String> {
    let store = store.clone();
    (move || {
...
pub fn list_skill_files(central_path: String) -> Result<Vec<SkillFileEntry>, String> {
    let path = std::path::PathBuf::from(&central_path);
    (move || {
        let entries = crate::skill_files::list_files(&path)?;
        Ok::<_, anyhow::Error>(
            entries
                .into_iter()
...
pub fn read_skill_file(central_path: String, file_path: String) -> Result<String, String> {
    let base = std::path::PathBuf::from(&central_path);
    crate::skill_files::read_file(&base, &file_path).map_err(format_anyhow_error)
}

pub fn cancel_current_operation(cancel: Arc<CancelToken>) -> Result<(), String> {
    cancel.cancel();
    Ok(())
}
