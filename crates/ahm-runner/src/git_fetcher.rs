...
pub fn clone_or_pull(
    ...
    {
        return checkout_commit(repo_url, dest, commit, cancel, proxy_url);
    }
...
pub fn clone_or_pull_sparse(
    ...
    // A preview identifies a commit, which is not a branch accepted by git clone --branch.
    if branch.is_some_and(|value| {
        value.len() == 40 && value.bytes().all(|byte| byte.is_ascii_hexdigit())
    }) {
        return clone_or_pull(repo_url, dest, branch, cancel, proxy_url);
    }
...
fn checkout_commit(
    ...
    if !dest.join(".git").exists() {
        operations.push(vec!["init", "."]);
        operations.push(vec!["remote", "add", "origin", repo_url]);
    }
    operations.push(vec!["fetch", "--depth=1", "origin", commit]);
    operations.push(vec!["checkout", "--detach", "FETCH_HEAD"]);
    for args in operations {
        let mut command = git_cmd(proxy_url);
        command.arg("-C").arg(dest).args(args);
        let output = run_cmd_with_timeout(
            command,
            Duration::from_secs(120),
            "Fetch the reviewed Git revision".to_owned(),
            cancel,
        )?;
        anyhow::ensure!(
            output.status.success(),
            "Fetching the reviewed Git revision failed: {}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
    let mut command = git_cmd(proxy_url);
    command.arg("-C").arg(dest).args(["rev-parse", "HEAD"]);
    let output = run_cmd_with_timeout(
        command,
        git_timeout(),
        "Read installed Git revision".to_owned(),
        cancel,
    )?;
    anyhow::ensure!(
        output.status.success(),
        "Unable to read installed Git revision"
    );
    let actual = String::from_utf8(output.stdout)?.trim().to_owned();
    anyhow::ensure!(
        actual.eq_ignore_ascii_case(commit),
        "Git revision does not match the reviewed skill"
    );
    Ok(actual)
}
