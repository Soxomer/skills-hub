...
use ahm_domain::{ContractValueError, IsoTimestamp, OrganizationId, ProjectId, ProjectInstanceId};
use rusqlite::{params, Connection, OptionalExtension};

const RUNNER_SCHEMA_VERSION: i64 = 1;
const RUNNER_SCHEMA_V1: &str = r#"
...
impl RunnerStateStore {
    ...
    fn migrate(&self) -> Result<(), RunnerStateError> {
        ...
        match version {
            0 => self.connection.execute_batch(RUNNER_SCHEMA_V1)?,
...
mod tests {
    ...
    #[test]
    fn project_instances_resolve_paths_only_inside_the_runner_store() {
        let store = RunnerStateStore::open_in_memory().unwrap();
