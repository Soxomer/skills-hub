...
const SkillDetailView = lazy(() => import('./components/skills/SkillDetailView'))
...
function SkillsHub({ onOpenWorkspace }: { onOpenWorkspace: () => void }) {
  ...
  const updateObjRef = useRef<WebUpdate | null>(null) as MutableRefObject<WebUpdate | null>
  ...
  const loadPlan = useCallback(async (showLoading = true) => {
    ...
    setError(null)
    try {
      const result = await invokeLibrary<OnboardingPlan>('get_onboarding_plan')
      setPlan(result)
  ...
  const loadDiscoveryScanSettings = useCallback(async () => {
    if (!runnerConnected) return null
    try {
      const result = await invokeLibrary<DiscoveryScanSettingsDto>(
  ...
  }, [invokeLibrary, runnerConnected])

  const loadManagedSkills = useCallback(async () => {
    try {
      const result = await invokeLibrary<ManagedSkill[]>('get_managed_skills')
  ...
  const loadTags = useCallback(async () => {
    try {
      const result = await invokeLibrary<TagWithCountDto[]>('get_tags')
      setTags(result)
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    }
  ...
  }, [runnerConnected, loadManagedSkills, loadTags])
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<string[]>('get_recent_projects')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<string>('get_central_repo_path')
      .then((path) => setStoragePath(path))
      .catch((err) => {
        setError(err instanceof Error ? err.message : String(err))
      })
  }, [runnerConnected, invokeLibrary])

  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<number>('get_git_cache_cleanup_days')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<number>('get_git_cache_ttl_secs')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<string>('get_github_token')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<GithubProxyConfigDto>('get_github_proxy_config')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<AutoUpdateConfigDto>('get_auto_update_config')
  ...
  useEffect(() => {
    if (!runnerConnected) return
    invokeLibrary<ToolConfigDto>('get_tool_config')
  ...
  const syncInstalledSkill = useCallback(
    async (
      ...
      for (let index = 0; index < jobs.length; index++) {
        ...
        try {
          await invokeLibrary('sync_skill_to_tool', {
  ...
  useEffect(() => {
    if (!runnerConnected || !githubProxyConfigLoaded) return
    let cancelled = false
  ...
  }, [githubProxyConfigLoaded, invokeLibrary, runnerConnected])
  ...
  useEffect(() => {
    ...
    const refreshAutoUpdateConfig = async () => {
      const config = await invokeLibrary<AutoUpdateConfigDto>('get_auto_update_config')
      if (cancelled) return
  ...
  const handlePickStoragePath = useCallback(async () => {
    try {
      if (!runnerConnected) {
        throw new Error(t('errors.runnerRequired'))
      }
      ...
      const newPath = await invokeLibrary<string>('set_central_repo_path', {
        path: selected,
      })
  ...
  }, [invokeLibrary, runnerConnected, loadManagedSkills, t, pickDirectory])
  const handleGitCacheCleanupDaysChange = useCallback(
    async (nextDays: number) => {
      ...
      if (!runnerConnected) return
      try {
        const updated = await invokeLibrary<number>('set_git_cache_cleanup_days', {
          days: normalized,
        })
  ...
  const handleGitCacheTtlSecsChange = useCallback(
    async (nextSecs: number) => {
      ...
      if (!runnerConnected) return
      try {
        const updated = await invokeLibrary<number>('set_git_cache_ttl_secs', {
          secs: normalized,
        })
  ...
  const handleGithubTokenChange = useCallback(
    async (nextToken: string) => {
      ...
      if (!runnerConnected) return
      try {
        await invokeLibrary('set_github_token', { token: nextToken })
      } catch (err) {
        setError(err instanceof Error ? err.message : String(err))
      }
    },
  ...
  const handleGithubProxyConfigChange = useCallback(
    async (enabled: boolean, port: number) => {
      ...
      if (!runnerConnected) return
      try {
        const saved = await invokeLibrary<GithubProxyConfigDto>(
        ...
        setError(err instanceof Error ? err.message : String(err))
        void invokeLibrary<GithubProxyConfigDto>('get_github_proxy_config')
          .then((config) => setGithubProxyConfig(config))
          .catch(() => {})
      }
    },
  ...
  const handleToolConfigChange = useCallback(
    async (nextConfig: ToolConfigDto) => {
      ...
      if (!runnerConnected) return true
      try {
        const saved = await invokeLibrary<ToolConfigDto>('set_tool_config', {
          config: nextConfig,
        })
        setToolConfig(saved)
        const status = await invokeLibrary<ToolStatusDto>('get_tool_status')
        ...
        try {
          const [config, status] = await Promise.all([
            invokeLibrary<ToolConfigDto>('get_tool_config'),
            invokeLibrary<ToolStatusDto>('get_tool_status'),
          ])
    ...
    [invokeLibrary, runnerConnected, t],
  )
  const handleAutoUpdateConfigChange = useCallback(
    async (
      ...
      if (!runnerConnected) return
      try {
        const updated = await invokeLibrary<AutoUpdateConfigDto>(
        ...
        setError(err instanceof Error ? err.message : String(err))
        void invokeLibrary<AutoUpdateConfigDto>('get_auto_update_config')
          .then((config) => setAutoUpdateConfig(config))
          .catch(() => {})
      }
    },
    [autoUpdateConfig?.enabled, invokeLibrary, runnerConnected, t],
  )
  const handleTriggerAutoUpdateTaskNow = useCallback(async () => {
    if (!runnerConnected || autoUpdateTriggering) return
    ...
    try {
      await invokeLibrary('trigger_auto_update_task_now_cmd')
      setSuccessToastMessage(t('autoUpdateTaskTriggered'))
      let sawRunning = false
      for (let attempt = 0; attempt < 30; attempt += 1) {
        await new Promise((resolve) => window.setTimeout(resolve, 2000))
        const latestConfig = await invokeLibrary<AutoUpdateConfigDto>(
  ...
  }, [autoUpdateTriggering, invokeLibrary, runnerConnected, loadManagedSkills, t])
  const handleClearGitCacheNow = useCallback(async () => {
    if (!runnerConnected) {
      setError(t('errors.runnerRequired'))
      return
    }
    try {
      const removed = await invokeLibrary<number>('clear_git_cache_now')
  ...
  }, [runnerConnected, t, pickDirectory])
  ...
  const handleSaveDiscoveryScanSettings = useCallback(
    async (disabledSourceKeys: string[]) => {
      ...
      try {
        const saved = await invokeLibrary<DiscoveryScanSettingsDto>(
    ...
    [invokeLibrary, runnerConnected, loadPlan, t],
  )
  ...
  const loadFeaturedSkills = useCallback(async () => {
    ...
    try {
      const result = await invokeLibrary<FeaturedSkillDto[]>('get_featured_skills')
  ...
  }, [featuredSkills.length, invokeLibrary])
  ...
  const handleExploreFilterChange = useCallback(
    (value: string) => {
      ...
      searchTimerRef.current = setTimeout(async () => {
        try {
          const results = await invokeLibrary<OnlineSkillDto[]>(
  ...
  const loadSkillPreview = useCallback(
    async (target: ExploreSkillPreviewTarget) => {
      ...
      try {
        const result = await invokeLibrary<GitSkillPreviewDto>('preview_git_skill_cmd', {
  ...
  const applySelectedAddModalTags = useCallback(
    async (skillId: string, skillName: string) => {
      ...
      try {
        await invokeLibrary('set_skill_tags', {
          skillId,
    ...
    [addModalTagIds, invokeLibrary, t],
  )

  const handleCancelLoading = useCallback(() => {
    setActionMessage(t('libraryTransport.cancelling'))
    void invokeLibrary('cancel_current_operation').catch(error => toast.error(String(error)))
  }, [invokeLibrary, t])
  ...
  const restoreSkillSavedTargets = useCallback(
    ...
    [installedToolIds, invokeLibrary, toolSupportsProjectScope],
  )
  ...
  const handleToggleSkillEnabled = useCallback(
    async (skill: ManagedSkill) => {
      ...
      try {
        await invokeLibrary('set_skill_enabled', {
          skillId: skill.id,
          enabled: nextEnabled,
        })
        if (nextEnabled) {
    ...
    [invokeLibrary, loadManagedSkills, restoreSkillSavedTargets, t],
  )
    ...
    try {
      ...
      for (const skill of bulkSelectedSkills) {
        ...
        for (const target of targetsToRemove) {
          ...
          try {
            await invokeLibrary('unsync_skill_from_tool', {
              skillId: skill.id,
              tool: target.tool,
              scope: skillScope,
  ...
  const handleConfirmBulkDelete = useCallback(async () => {
    ...
    try {
      for (let index = 0; index < bulkSelectedSkills.length; index++) {
        ...
        try {
          await invokeLibrary('delete_managed_skill', { skillId: skill.id })
        } catch (err) {
          errors.push({
            title: skill.name,
            message: err instanceof Error ? err.message : String(err),
  ...
  const handleCreateTag = useCallback(
    async (name: string) => {
      try {
        await invokeLibrary('create_tag', { name })
        await loadTags()
    ...
    [invokeLibrary, loadTags, t],
  )

  const handleRenameTag = useCallback(
    async (tagId: number, name: string) => {
      try {
        const renamed = await invokeLibrary<{ id: number; name: string }>('rename_tag', {
          tagId,
          name,
        })
        setSelectedTagIds((current) =>
    ...
    [invokeLibrary, loadManagedSkills, loadTags, t],
  )
  ...
  const handleConfirmDeleteTag = useCallback(async () => {
    ...
    try {
      ...
      await invokeLibrary('delete_tag', { tagId: pendingDeleteTag.id })
  ...
  }, [invokeLibrary, loadManagedSkills, loadTags, pendingDeleteTag, t])
  ...
  const handleSaveSkillTags = useCallback(
    async (skill: ManagedSkill, tagIds: number[]) => {
      try {
        ...
        await invokeLibrary('set_skill_tags', { skillId: skill.id, tagIds })
        await loadManagedSkills()
        await loadTags()
    ...
    async (nextProjects: SetStateAction<string[]>) => {
      ...
      try {
        ...
        for (const projectPath of addedProjects) {
          savedProjects = await invokeLibrary<string[]>('save_recent_project', {
            projectPath,
          })
        }
        if (
    ...
    try {
      ...
      for (const group of plan.groups) {
        ...
        try {
          ...
          installResult = await invokeLibrary<{
            skill_id: string
            central_path: string
  ...
  const handleCreateLocal = async () => {
    ...
    try {
      ...
      const candidates = await invokeLibrary<LocalSkillCandidate[]>(
      ...
      if (candidates.length === 1 && candidates[0].valid) {
        ...
        const created = await invokeLibrary<InstallResultDto>(
  ...
  const handleCreateGit = async () => {
    ...
    try {
      ...
      if (isFolderUrl) {
        const candidates = await invokeLibrary<GitSkillCandidate[]>(
          'list_git_skills_cmd',
          { repoUrl: url },
        )
        if (candidates.length === 0) {
  ...
  const handleInstallPreviewSkill = useCallback(() => {
    ...
    handleExploreInstall(skillPreview?.source_url ?? target.source_url, target.name)
  }, [handleCloseSkillPreview, handleExploreInstall, skillPreviewTarget, skillPreview])
  ...
  const handleScopeChange = useCallback(
    async (nextScope: 'global' | 'project', nextProjects: string[]) => {
      ...
      try {
        ...
        if (nextScope === 'project') {
          for (const projectPath of projects) {
            const saved = await invokeLibrary<string[]>('save_recent_project', {
              projectPath,
            })
  ...
  const handlePickProject = useCallback(async () => {
    try {
      if (!runnerConnected) throw new Error(t('errors.runnerRequired'))
      const selected = await pickDirectory({
        directory: true,
        multiple: false,
  ...
  const handleUpdateManaged = useCallback(
    ...
    try {
      ...
      await invokeLibrary<UpdateResultDto>('update_managed_skill', { skillId: skill.id })
    ...
    [invokeLibrary, loadManagedSkills, t],
  )
  ...
  return (
    <div className={`skills-app${sidebarCollapsed || narrowWindow ? ' sidebar-collapsed' : ''}`}>
      <Toaster
        position="top-right"
        richColors
      ...
      <Header
        ...
        collapsed={sidebarCollapsed || narrowWindow}
      ...
      <main className="skills-main">
        {activeView === 'detail' && detailSkill ? (
          <Suspense fallback={<p role="status">{t('loading')}</p>}><SkillDetailView
            skill={detailSkill}
            onBack={handleBackToList}
            invokeLibrary={invokeLibrary}
